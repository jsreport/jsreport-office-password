  ...
  reporter.initializeListeners.add(definition.name, () => {
    reporter.documentStore.collection('templates').beforeInsertListeners.add(definition.name, async (doc, req) => {
      ...
      try {
        ...
        if (e.encryptionNoSecret) {
          e.message = 'office-password uses encryption to store sensitive data and needs secret key to be defined to be able to create entity, make sure to define the "options.encryption.secretKey" option in config'
        }
    ...
    reporter.documentStore.collection('templates').beforeUpdateListeners.add(definition.name, async (q, u, req) => {
      ...
      try {
        ...
        if (e.encryptionNoSecret) {
          e.message = 'office-password uses encryption to store sensitive data and needs secret key to be defined to be able to update entity, make sure to define the "options.encryption.secretKey" option in config'
        }
    ...
    reporter.afterRenderListeners.insert({ before: 'scripts' }, definition.name, async (req, res) => {
      if (!req.template.officePassword || req.template.officePassword.enabled !== true) {
        return
      }
      ...
      if (req.template.officePassword.passwordRaw) {
        ...
        try {
          ...
          if (e.encryptionNoSecret) {
            e.message = 'office-password uses encryption to store sensitive data and needs secret key to be defined to be able to decrypt stored values, make sure to define the "options.encryption.secretKey" option in config'
          } else if (e.encryptionDecryptFail) {
            e.message = 'office-password data decrypt failed, looks like secret key value is different to the key used to encrypt sensitive data, make sure "options.encryption.secretKey" was not changed'
          }
